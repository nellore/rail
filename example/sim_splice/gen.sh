...
python $SIM/splice_sim.py --fasta=$GENOME --gtf=$ANNOTATIONS --output-prefix=$PWD/fly --num-xscripts=100 --num-nucs=10000000 --alternative-spliced=0 --stranded=1 --readmm_rate=0.01 --snp_rate=0 --indel_rate=0 --variants_file=$DRIVERS/variants.txt --chrsizes=$FASTAIDX --output-annotations=$ANNOT_SITES --paired-end=1
