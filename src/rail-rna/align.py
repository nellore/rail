...
global, args is also an argument of the go() function so different
...
def unmapped_region_split(unmapped_seq, left_reference_seq,
        right_reference_seq):
    left_score_matrix = needlemanWunsch.needlemanWunsch(
            left_reference_seq, unmapped_seq,
            needlemanWunsch.matchCost() 
        )[1]
    right_score_matrix = needlemanWunsch.needlemanWunsch(
            right_reference_seq[::-1], unmapped_seq[::-1],
            needlemanWunsch.matchCost()
        )[1][::-1,::-1]
    total_score_matrix = left_score_matrix + right_score_matrix
    argmax_total = np.argmax(total_score_matrix)
    try:
        return needlemanWunsch.medianTieBreaker(
                total_score_matrix, argmax_total / (len(unmapped_seq) + 1)
           )
    except Exception as e:
        print >>sys.stderr,'check the nw algo'
        print >>sys.stderr,total_score_matrix.shape
        print >>sys.stderr,argmax_total
        sys.exit(1)

def unmapped_region_similarity(unmapped_seq, reference_seq):
    score, score_matrix = needlemanWunsch.needlemanWunsch(
            reference_seq, unmapped_seq, needlemanWunsch.matchCost()
           )
    return score
...
def exons_and_introns_from_read(fasta_object, read_seq, readlets, 
    ...
    """Composes a given read's aligned readlets and returns ECs and introns.
       ...
       min_seq_similarity: if the difference in length between an unmapped
                           ...
                           needlemanWunsch.matchCost() for the similarity 
                           matrix used.
    ...
    for strand in composed:
        ...
        for pos, end_pos, displacement in composed[strand][1:]:
            ...
            if not read_distance:
                ...
                if abs(discrepancy) <= max_discrepancy:
                    if unmapped_region_similarity(
                        current_read_seq[unmapped_displacement:displacement],
                        fasta_object.fetch_sequence(rname, last_end_pos, 
                            pos - 1)
                    ) >= min_seq_similarity * read_distance:
                    ...
                    between ECs.'''
                    assert read_distance 
                    split = \
                        unmapped_region_split(
                            current_read_seq[unmapped_displacement:\
                                displacement], 
                            fasta_object.fetch_sequence(rname,
                                last_end_pos, last_end_pos + read_distance - 1), 
                            fasta_object.fetch_sequence(rname,
        ...
        if unmapped_displacement != read_seq_size:
            if unmapped_region_similarity(
                ...
                ) >= min_seq_similarity * unmapped_base_count:
                # Fill
...
class BowtieOutputThread(threading.Thread):
    ...
    def __init__(self, input_stream, fasta_object, done_bowtie, 
        readletized=False, write_unmapped=True, 
        unmapped_stream=open(os.devnull, 'w'), output_stream=sys.stdout, 
        exon_differentials=True, exon_intervals=False, verbose=False, 
        bin_size=10000, min_intron_size=5, min_strand_readlets=1,
        max_discrepancy=2, min_seq_similarity=0.5, max_intron_size=100000,
        ...
        """Constructor for BowtieOutputThread.
           ...
           fasta_object: object of class fasta.fasta corresponding to 
                         FASTA reference; used for realignment of unmapped
                         regions between exonic chunks.
           done_bowtie: threading.Event() object that is set by thread
                        when Bowtie output is complete
           ...
           verbose: True if alignments should occasionally be written 
                    to stderr.
           bin_size: genome is partitioned in units of bin_size.
           min_intron_size: introns smaller than this number of bases are 
                            filtered out.
    ...
    def run(self):
        """Prints exons for reads and exons/introns for readlets.
           ...
           Genome partitioned in units of self.bin_size.
                        ...
                        if intron_end_pos - intron_pos > self.max_intron_size:
                            if self.verbose: 
                                print >> sys.stderr, 'Intron of size > ' \
                                    'max-intron-size filtered at %s:%d-%d' \
        ...
        self.done_bowtie.set()

def go(reference_file, bowtie_exe="bowtie", bowtie_index_root="genome",
    bowtie_args=None, temp_dir_path=tempfile.mkdtemp(), archive=False, 
    bin_size=10000, verbose=False, read_filename='reads.tsv', 
    readlet_filename='readlets.tsv', unmapped_filename='unmapped.tsv', 
    exon_differentials=True, exon_intervals=False, min_readlet_size=5, 
    max_readlet_size=25, readlet_interval=5, capping_fraction=.75, 
    min_strand_readlets=1, max_discrepancy=2, min_seq_similarity=0.5, 
    min_intron_size=5, max_intron_size=100000, report_multiplier=1.2):
    """Runs Rail-RNA-align.
    ...
    with open(unmapped_filename, 'w') as unmapped_stream:
        output_thread = BowtieOutputThread(
                bowtie_process.stdout, fasta_object, done_bowtie, 
                readletized=False, write_unmapped=True, 
    ...
    # TODO: Count number of lines output
    print >> sys.stderr, 'DONE with align.py; in/out = %d/%d; ' \
        'time=%0.3f secs' % (0, 0, time.time()-start_time)
...
if not args.test:
    ...
    go(args.refseq, bowtie_exe=args.bowtie_exe,
        bowtie_index_root=args.bowtie_idx,
