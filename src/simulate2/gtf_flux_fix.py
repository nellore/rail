...
parser = argparse.ArgumentParser(description='Fix GTF file in preparation for Flux Simulator')

parser.add_argument('--gtf', metavar='PATH', type=str, required=True, nargs='+', help='Path to GTF file to fix')
parser.add_argument('--fasta', metavar='PATH', type=str, required=True, nargs='+',
...
def transform_attributes(attr):
    """ Move transcript_id into the first attribute position.  A
        necessary preliminary step for flux's sort function. """
    ...
    with open(gtffn, 'r') as gtffh:
        for line in gtffh:
            ...
            if refid not in fasta_names:
                ...
                continue
            print '\t'.join([refid, source, feat, st, en, score, strand, frame, transform_attributes(attr)])
