#!/usr/bin/env python
"""
generate_geuvadis_sims.py
Abhi Nellore / November 28, 2014
Based heavily on generate_bioreps.py

Generates 660 GEUVADIS sims for Rail paper using Flux Simulator by:

1) starting a single Flux simulation and interrupting the pipeline after
    6 columns of a PRO file P have been written.
2) reading RPKMs from all the GEUVADIS data.
3) writing 660 files, each a copy of P except with a different coverage
...
if __name__ == '__main__':
    ...
    return_values = []
    with open(args.rpkm) as sample_stream:
        samples = sample_stream.readline().strip().split('\t')[4:]
